...
const rule = {
    meta: {
        type: "suggestion",
        docs: {
            description: "Encourage proper resource hints (preload, prefetch, preconnect) for better loading performance",
        },
        schema: [],
    },
    create(context) {
        ...
        function getResourceType(url) {
            ...
            if (CRITICAL_RESOURCE_PATTERNS.fonts.some((ext) => urlLower.includes(ext))) {
                return "font";
            }
            if (CRITICAL_RESOURCE_PATTERNS.images.some((ext) => urlLower.includes(ext))) {
                return "image";
            }
            if (CRITICAL_RESOURCE_PATTERNS.scripts.some((ext) => urlLower.includes(ext))) {
                return "script";
            }
            if (CRITICAL_RESOURCE_PATTERNS.styles.some((ext) => urlLower.includes(ext))) {
                return "style";
            }
            if (CRITICAL_RESOURCE_PATTERNS.videos.some((ext) => urlLower.includes(ext))) {
                return "video";
            }
            return null;
        }
        ...
        function checkResourceHints(node, text) {
        ...
        function checkExistingHints(node, text) {
        ...
        return {
            ...
            JSXOpeningElement(node) {
                if (node.name?.type === "JSXIdentifier") {
                    ...
                    if (tagName === "link") {
                        ...
                        const hrefAttr = node.attributes?.find((a) => a.name?.name === "href");
                        const asAttr = node.attributes?.find((a) => a.name?.name === "as");
                        if (relAttr && hrefAttr) {
            ...
            "Program:exit"() {
                ...
                const hasGoogleFonts = Array.from(foundResources).some((url) => url.includes("fonts.googleapis.com"));
                const hasGoogleFontsPreconnect = Array.from(foundHints).some((hint) => hint.includes("preconnect") && hint.includes("fonts.googleapis.com"));
                if (hasGoogleFonts && !hasGoogleFontsPreconnect) {
                    context.report({
                        node: context.getSourceCode().ast,
                        message: "Google Fonts detected but missing preconnect hints. Add <link rel='preconnect' href='https://fonts.googleapis.com'> and <link rel='preconnect' href='https://fonts.gstatic.com' crossorigin>",
                    });
                }
            },
        };
