const rule = {
    meta: {
        type: "problem",
        docs: {
            description: "Prevent common memory leak patterns in JavaScript/TypeScript applications",
        },
        schema: [],
    },
    create(context) {
        ...
        function checkClosureReferences(node) {
        ...
        return {
            ...
            CallExpression(node) {
                ...
                if (node.callee?.type === "MemberExpression") {
                    ...
                    if (obj?.name === "document" &&
                        prop?.name &&
                        ["querySelector", "querySelectorAll", "getElementById"].includes(prop.name)) {
            ...
            MemberExpression(node) {
                if (node.property?.name === "subscribe" &&
