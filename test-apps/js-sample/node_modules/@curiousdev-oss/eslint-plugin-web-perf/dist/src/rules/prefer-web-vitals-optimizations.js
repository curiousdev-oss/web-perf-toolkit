...
const rule = {
    meta: {
        type: "suggestion",
        docs: {
            description: "Enforce Core Web Vitals optimizations for better Lighthouse performance scores",
        },
        schema: [],
    },
    create(context) {
        function checkImageOptimizations(node, attributes) {
            const attrs = new Map();
            ...
            if (hasSrc) {
                ...
                if (!hasLoading) {
                    context.report({
                        node,
                        message: "Add loading attribute (loading='lazy' for below-the-fold, loading='eager' for above-the-fold) to optimize LCP",
                    });
                }
                ...
                if (!hasWidth || !hasHeight) {
                    context.report({
                        node,
                        message: "Set width and height attributes to prevent Cumulative Layout Shift (CLS)",
                    });
                }
                ...
                if (!hasSrcSet) {
                    context.report({
                        node,
                        message: "Consider using srcset for responsive images to improve LCP on different device sizes",
                    });
                }
                ...
                if (!hasAlt) {
                    context.report({
                        node,
                        message: "Add alt attribute for accessibility and better Lighthouse score",
                    });
                }
            }
        }
        function checkFontOptimizations(node) {
            ...
            if (source.includes("@font-face") && !source.includes("font-display")) {
                context.report({
                    node,
                    message: "Add font-display: swap to @font-face to improve CLS and LCP",
                });
            }
            ...
            if (source.includes("fonts.googleapis.com") ||
                ...
                if (!source.includes("display=swap")) {
                    context.report({
                        node,
                        message: "Add &display=swap to Google Fonts URLs to optimize font loading",
                    });
                }
            }
        }
        function checkEventListenerOptimizations(node) {
                ...
                if (args?.length >= 2) {
                    ...
                    if (eventType?.type === "Literal") {
                        ...
                        if (passiveEvents.includes(event)) {
                            const hasPassive = options?.type === "ObjectExpression" &&
                                options.properties?.some((prop) => prop.key?.name === "passive" && prop.value?.value === true);
                            if (!hasPassive) {
                                context.report({
                                    node,
        ...
        function checkAsyncOperations(node) {
        ...
        function checkPreloadHints(node) {
            const source = context.getSourceCode().getText(node);
            ...
            if (source.includes("<link")) {
                ...
                if (source.includes('href="') && source.includes(".woff")) {
                    if (!hasPreload) {
                        context.report({
                            node,
                            message: "Preload critical fonts with <link rel='preload'> to improve LCP",
                        });
                    }
                }
                if (source.includes('src="') &&
                    ...
                    if (!hasPreload &&
                        ...
                        context.report({
                            node,
                            message: "Preload hero/banner images with <link rel='preload'> to improve LCP",
                        });
                    }
                }
            }
        ...
        return {
                    ...
                    if (tagName === "div") {
                        ...
                        if (styleAttr) {
                            ...
                            if (styleValue.includes("background-image")) {
                                context.report({
                                    node,
                                    message: "Background images on divs can hurt LCP. Consider using <img> with object-fit for better optimization",
                                });
                            }
                        }
                    }
                ...
                if (imgMatches) {
                    imgMatches.forEach(() => {
                        ...
                        if (!source.includes("loading=")) {
                            context.report({
                                node,
                                message: "Add loading attribute to images in templates for LCP optimization",
                            });
                        }
            ...
            ImportDeclaration(node) {
                ...
                if (source.includes("performance") || source.includes("analytics")) {
                    context.report({
                        node,
                        message: "Consider importing 'web-vitals' library to monitor Core Web Vitals in production",
                    });
                }
            },
        };
