...
const rule = {
    meta: {
        type: "problem",
        docs: {
            description: "Prevent render-blocking resources that hurt First Contentful Paint and Largest Contentful Paint",
        },
        schema: [],
    },
    create(context) {
        function checkRenderBlockingCSS(node) {
            ...
            if (source.includes("import") && source.includes(".css")) {
                context.report({
                    node,
                    message: "CSS imports can block rendering. Consider using CSS-in-JS, dynamic imports, or ensuring critical CSS is inlined",
                });
            }
            ...
            if (source.includes("<link") && source.includes('rel="stylesheet"')) {
                if (!source.includes("media=") && !source.includes("disabled")) {
                    context.report({
                        node,
                        message: "Stylesheet links without media queries block rendering. Consider using media='print' onload pattern or critical CSS",
                    });
                }
            }
            ...
            if (source.includes("@import")) {
                context.report({
                    node,
                    message: "@import statements block rendering. Use bundler imports or <link> tags instead",
                });
            }
        }
        function checkRenderBlockingFonts(node) {
            const source = context.getSourceCode().getText(node);
            ...
            if (source.includes("fonts.googleapis.com")) {
                if (!source.includes("display=swap") &&
                    ...
                    context.report({
                        node,
                        message: "Google Fonts without display=swap block rendering. Add &display=swap to the URL or font-display: swap to CSS",
                    });
                }
                if (!source.includes("rel=preconnect")) {
                    context.report({
                        node,
                        message: "Add <link rel='preconnect' href='https://fonts.googleapis.com'> to reduce font loading delays",
                    });
                }
            }
            ...
            if (source.includes("@font-face") && !source.includes("font-display")) {
                context.report({
                    node,
                    message: "Add font-display: swap to @font-face to prevent invisible text during font swap",
                });
            }
        }
        function checkRenderBlockingScripts(node) {
            const source = context.getSourceCode().getText(node);
            ...
            if (source.includes("<script src=") &&
                !source.includes("async") &&
                !source.includes("defer")) {
                context.report({
                    node,
                    message: "External scripts without async/defer block rendering. Add async for third-party scripts or defer for your scripts",
                });
            }
            ...
            if (scriptMatches) {
                scriptMatches.forEach((script) => {
                    if (script.length > 500) {
                        ...
                        context.report({
                            node,
                            message: "Large inline scripts block rendering. Consider moving to external files with async/defer",
                        });
                    }
                });
            }
        ...
        return {
            ...
            ImportDeclaration(node) {
                ...
                const isHeavyLibrary = heavyLibraries.some((lib) => source.includes(lib));
                if (isHeavyLibrary) {
                    context.report({
                        node,
            ...
            CallExpression(node) {
                ...
                if (node.callee?.name === "eval") {
                    context.report({
                        node,
                        message: "eval() blocks JavaScript parsing and execution. Avoid or use JSON.parse for safe parsing",
                    });
                }
                if (node.callee?.name === "Function" && node.arguments?.length > 0) {
                    context.report({
                        node,
                        message: "Function constructor blocks execution. Use regular functions or arrow functions",
                    });
                }
                ...
                if (node.callee?.type === "MemberExpression" &&
                    ...
                    context.report({
                        node,
                        message: "Synchronous XMLHttpRequest blocks the main thread. Use async requests or fetch()",
                    });
                }
            },
            // JSX elements
            JSXOpeningElement(node) {
                if (node.name?.type === "JSXIdentifier") {
                    ...
                    // Check for link tags
                    if (tagName === "link") {
                        const relAttr = node.attributes?.find((a) => a.name?.name === "rel");
                        const mediaAttr = node.attributes?.find((a) => a.name?.name === "media");
                        if (relAttr?.value?.value === "stylesheet" && !mediaAttr) {
                            context.report({
                                node,
                                message: "Stylesheet <link> without media attribute blocks rendering. Add media='all' or specific media query",
                            });
                        }
                    }
                    // Check for script tags
                    if (tagName === "script") {
                        const srcAttr = node.attributes?.find((a) => a.name?.name === "src");
                        const asyncAttr = node.attributes?.find((a) => a.name?.name === "async");
                        const deferAttr = node.attributes?.find((a) => a.name?.name === "defer");
                        if (srcAttr && !asyncAttr && !deferAttr) {
                            context.report({
                                node,
                                message: "External <script> without async/defer blocks rendering. Add async or defer attribute",
                            });
                        }
                    }
                }
            ...
            Property(node) {
                if (node.key?.name === "style" &&
                    ...
                    if (node.value.properties?.length > 20) {
                        context.report({
                            node,
                            message: "Large inline style objects can hurt performance. Consider CSS classes or styled-components",
                        });
                    }
                }
            },
