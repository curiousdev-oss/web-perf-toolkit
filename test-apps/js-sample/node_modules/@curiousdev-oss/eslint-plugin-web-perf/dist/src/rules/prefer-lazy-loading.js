const rule = {
    ...
    create(context) {
        function checkImageLazyLoading(node, attributes) {
            const attrs = new Map();
        ...
        return {
            ...
            ImportDeclaration(node) {
                ...
                const isHeavyLibrary = heavyLibraries.some(lib => source.includes(lib) || source.startsWith(lib));
                if (isHeavyLibrary && node.specifiers?.length > 0) {
                    // Check if it's a default or namespace import (likely heavy)
                    const hasHeavyImport = node.specifiers.some((spec) => spec.type === "ImportDefaultSpecifier" ||
