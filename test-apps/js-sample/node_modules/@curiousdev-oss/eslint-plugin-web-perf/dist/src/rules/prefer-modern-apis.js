...
const rule = {
    meta: {
        type: "suggestion",
        docs: {
            description: "Encourage modern web APIs over legacy ones for better performance and compatibility",
        },
        schema: [],
        fixable: "code",
    },
    create(context) {
        function checkLegacyAPI(node, apiName) {
        ...
        function checkModernAlternative(node, apiName) {
        ...
        return {
            ...
            NewExpression(node) {
                ...
                if (node.callee?.name === "Date" && node.arguments?.length === 0) {
                    context.report({
                        node,
                        message: "Use Date.now() instead of new Date() for timestamps - it's faster",
                        fix(fixer) {
            ...
            BinaryExpression(node) {
                ...
                if (node.operator === "!==" &&
                    ...
                    context.report({
                        node,
                        message: "XMLHttpRequest feature detection suggests legacy code. Consider using fetch() with polyfills.",
                    });
                }
                ...
                if (node.operator === "===" &&
                    ...
                    context.report({
                        node,
                        message: "Fetch feature detection found. Ensure you're using modern bundling with polyfills.",
                    });
                }
            },
            ...
            ImportDeclaration(node) {
                ...
                const isPolyfill = polyfillPatterns.some((pattern) => source.includes(pattern));
                if (isPolyfill) {
                    context.report({
                        node,
